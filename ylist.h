/**
 * ylist
 * This is easy-to-use list compared with 'ylistl'. But, in performance point of view, this is slower.
 * Interface of 'ylist' may return not-essential-value for easy-to-use; For example, parameter value itself.
 *
 ...
 * Instead of that, we uses 'ylist_walker_next_forward/ylist_walker_next_backward'
 *  to save one-indirect-function-call-cost, because next_forward/next_backward doesn't do much.
 * (on-indirect-function-call-cost is pretty high compare with what are done inside function.)
 */
struct ylist_walker {
    struct ylist*           __list;
...
ylist_free_item(struct ylist* l, void* item) {
    if(NULL != l->__freecb) { l->__freecb(item); }
    else {yfree(item); }
}
...
ylist_destroy(struct ylist* l) {
    ...
    ylistl_foreach_item_removal_safe(p, n, &l->__head, struct ylist_node, __link) {
...
ylist_walker_create(struct ylist* l, int type) {
    struct ylist_walker* w = (struct ylist_walker*)ymalloc(sizeof(struct ylist_walker));
    w->__list = l;
    w->__curr = &l->__head;
    switch(type) {
