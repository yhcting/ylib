...
_tree_walker_preot_next(struct _tree_walker_preot* p) {
	...
	ylistl_foreach_backward(pos, &p->base.wb.__next->_child) {
		ystack_push(p->s,
			    container_of(pos, struct ytreel_link, _sibling));
	}
	...
	return (p->base.wb.__curr)?
		container_of(p->base.wb.__curr, struct ytree_node, __link):
		NULL;;
}
...
_tree_walker_r2lpreot_next(struct _tree_walker_preot* p) {
	...
	p->base.wb.__curr = p->base.wb.__next;
	p->base.wb.__next = ystack_is_empty(p->s)?
			    NULL:
			    (struct ytreel_link*)ystack_pop(p->s);
...
_tree_walker_levelot_next(struct _tree_walker_levelot* p) {
	...
	p->base.wb.__curr = p->base.wb.__next;
	p->base.wb.__next = yqueue_is_empty(p->q)?
			    NULL:
...
ytree_walker_create(const struct ytree_node* top_node, int type) {
	...
	switch(type) {
		...
		p = (struct _tree_walker_preot*)
			ymalloc(sizeof(struct _tree_walker_preot));
		/*
		...
		if (YTREE_WALKER_PRE_OT == type)
			p->base.wb.next = (void*)&_tree_walker_preot_next;
		else
			p->base.wb.next = (void*)&_tree_walker_r2lpreot_next;
		...
		p = (struct _tree_walker_levelot*)
			ymalloc(sizeof(struct _tree_walker_levelot));
		/* This stack SHOULD NOT be used to destroy item */
		p->q = yqueue_create(NULL);
		p->base.wb.next = (void*)&_tree_walker_levelot_next;
		p->base.wb.__next = &top_node->__link;
		w = (struct _tree_walker_base*)p;
        } break;
		...
		p = (struct _tree_walker_postot*)
			ymalloc(sizeof(struct _tree_walker_postot));
		/*
		 * This stack SHOULD NOT be used to destroy item.
		   Just set freecb as NULL
		*/
		...
		if (YTREE_WALKER_POST_OT == type) {
			p->base.wb.next = (void*)&_tree_walker_postot_next;
			/*
			...
			p->base.wb.next = (void*)&_tree_walker_r2lpostot_next;
			/*
			 * find initial(starting) value
			 * of post-order traverse
			 */
