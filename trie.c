...
_alloc_node() {
    _node_t* n = ymalloc(sizeof(_node_t));
    if(!n) {
        /*
...
ytrie_walk(struct ytrie* t, void* user,
    ...
    yassert(t && from);
    if(n) {
        return _walk_internal(user, n, cb, (unsigned char*)buf, YTRIE_MAX_KEY_LEN, 0);
    } else { return -1; }
}
...
ytrie_insert(struct ytrie* t, const unsigned char* key, unsigned int sz, void* v) {
    ...
    if(!v || (sz >= YTRIE_MAX_KEY_LEN) ) { return -1; /* error case */ }
...
struct ytrie*
ytrie_clone(const struct ytrie* t, void* user, void*(*clonev)(void*, const void*)) {
...
#define YTRIEFail   2
    ...
    while(n) {
        ...
        if( cnt > 1 || ((1 == cnt) && n->v) ) { ret = YTRIEBranch; break; }
        else if(0 == cnt) { yassert(n->v); ret = YTRIELeaf; break; }
        c = j<<4;
        n = n->n[j];
        ...
        if(cnt > 1) { ret = YTRIEBranch; break; }
        else if(0 == cnt) {
